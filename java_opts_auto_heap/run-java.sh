...
#                     '-Xmx'.
#                     It defaults to "35" when the maximum amount of memory available
#                     to the container is below 300M, otherwise defaults to "80".
...
calc_max_memory() {
  ...
  if [ -n "${JAVA_MAX_MEM_RATIO:-}" ]; then
    ...
    calc_mem_opt "${CONTAINER_MAX_MEMORY}" "35" "mx"
  else
    calc_mem_opt "${CONTAINER_MAX_MEMORY}" "80" "mx"
  fi
}
